<?php
    ...
    use app\mobile\model\Zhuopin as ZhuopinModel;
    class User extends \think\Controller
    {
        ...
        public function UserRegitSend($verify,$sphone){
            ...
            if($sphone==""){
                ...
                $this->success('发送成功，请稍后查看验证码','Mobile/User/Regit','','1');
            }         
        }
        ...
        public function RegitSecond(){
            if ($this->request->isPost()) {
                ...
                $this->error('操作失败', 'Mobile/User/Regit');
            }
                ...
                if($_POST['verify']<>""){
                    ...
                    if ($this->request->isPost()) {
                        ...
                        if ($this->user_model->allowField(true)->save($data)) {
                            ...
                            $this->success('注册成功', 'Mobile/User/UserContole');
                        } else {
                            $this->error('保存失败');
                        }
                    }
        ...
        public function UserContole(){
            ...
            return $this->fetch('UserContole',compact('Result'));
        }
        ...
        public function Game(){
            ...
            $videoCounta=DB::name("video")->where(array('v_sel'=>'1'))->count();//范课数量
            $videoCountb=DB::name("video")->where(array('v_sel'=>'2'))->count();//亲子课数量

            $ZpCount=DB::name("zhuopin")->where(array('user_id'=>$user_id))->count();//范课订单数

            $bga=($orderCounta/$videoCounta)*100;//范课学习比例
            $bgb=($orderCountb/$videoCountb)*100;//亲子课学习比例
            return $this->fetch('Game',compact('userList','orderCounta','orderCountb','videoCounta','videoCountb','ZpCount','bga','bgb'));
        }
        /**
         * 删除未支付的订单
         */
